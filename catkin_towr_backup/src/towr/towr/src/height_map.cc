...
HeightMap::MakeTerrain (TerrainID type)
{
  switch (type) {
    ...
    // case CombinedTerrainID:      return std::make_shared<CombinedTerrain>(0,0.3,0.2); break;

    case SlopeWithRollPitchID:      return std::make_shared<SlopeWithRollPitch>(0.65,0.4,0.4); break;
    // case SlopeWithRollPitchID:      return std::make_shared<SlopeWithRollPitch>(0,0.1,0.0); break;              // only pitch

    case SlopeWithImpulseID:        return std::make_shared<SlopeWithImpulse>(0.65,0.4,0.4);  break;                   

    case SlopeWithStepID:           return std::make_shared<SlopeWithStep>(0.65,0.5,0.5);  break;           
    // case SlopeWithStepID:           return std::make_shared<SlopeWithStep>(0,0.1,0.0);  break;                      // only pitch
    case SlopeWithSmallGapID:       return std::make_shared<SlopeWithSmallPit>(0.65,0.2,0.2);  break;
    case SlopeWithLargeGapID:       return std::make_shared<SlopeWithLargePit>(0.65,0.2,0.2);  break;
